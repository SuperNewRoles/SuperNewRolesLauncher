...
async function refreshLocalPresets(keepStatusMessage = false): Promise<void> {
  ...
  try {
    ...
    renderLocalPresetList();
    if (!keepStatusMessage) {
      setStatusLine(presetStatus, t("preset.statusLoadFailed", { error: String(error) }), "error");
    }
  } finally {
    presetLoading = false;
    updateButtons();
...
presetExportButton.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatusLine(presetStatus, t("preset.statusExportFailed", { error: String(error) }), "error");
  } finally {
...
presetInspectButton.addEventListener("click", async () => {
  ...
  try {
    ...
    archivePresets = [];
    renderArchivePresetList();
    setStatusLine(presetStatus, t("preset.statusInspectFailed", { error: String(error) }), "error");
  } finally {
...
presetImportButton.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatusLine(presetStatus, t("preset.statusImportFailed", { error: String(error) }), "error");
  } finally {
